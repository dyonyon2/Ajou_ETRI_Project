...
//aodv
	...
	#define HELLO_TIMEOUT_INTERVAL 20000 
	...
	//22 Bytes인데 sizeof 하면 24 Bytes		need to sub 2
	typedef struct {
	    uint8_t type;			//Msg type
...
// NMS Data
	...
	struct nms_table_list{			//						=> nms_table이 12개 => 12 * 104 byte = 1,248 byte
...
void Make_msg_format_RANN(void);
...
void rt_table_update(struct rt_table *rt, ll_addr next, uint8_t hcnt, struct timeval *now,float alm, uint32_t seqno);
int rt_table_get_index(ll_addr dest_addr);
int rt_table_insert(ll_addr dest_addr, ll_addr next, ll_addr source_addr, uint8_t prev_exist, ll_addr prev_addr, uint8_t hcnt, uint32_t IPaddr, uint32_t hello_cnt, struct timeval *now, float alm, uint32_t seqno);
